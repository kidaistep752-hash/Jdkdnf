-- ====================================================================
--  BLOCK 1: SERVICES, CONFIG & MEMORY CLEANUP
-- ====================================================================
...
-- Очистка старых копий UI
if CoreGui:FindFirstChild("SimpleNeonSense") then
    CoreGui.SimpleNeonSense:Destroy()
end

-- Удаление зависших сил полета
if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
    local root = LocalPlayer.Character.HumanoidRootPart
    for _, child in pairs(root:GetChildren()) do
        if child:IsA("BodyVelocity") or child:IsA("BodyGyro") or child.Name == "FlyVelocity" then
            child:Destroy()
        end
    end
end
...
_G.Config = {
    ...
    Noclip = false, SelectedPlayer = "", Target = false,
    GodModeToggle = false, GodModeType = "Loop",
    SpinBot = false, SpinSpeed = 30,
    FlyMode = false, FlySpeedValue = 40,
    XNeoFlyActive = false, OnePunchFling = false,
    ESPToggle = false, ESPColor = "WHITE", MM2Mod = false
}
...
-- ====================================================================
--  BLOCK 2: UI BASE CREATION & DRAG SYSTEM
-- ====================================================================
...
-- ====================================================================
--  BLOCK 4: CORE PLAYER CHEATS LOGIC (MOBILE FRIENDLY FLY)
-- ====================================================================
local RunService = game:GetService("RunService")
...
RunService.Stepped:Connect(function()
    ...
    -- 4. ПОЛЕТ ДЛЯ СЕНСОРНОГО ЭКРАНА (Куда повернута камера + куда двигаете джойстик)
    if _G.Config.FlyMode then
        ...
        -- Считываем движение мобильного джойстика через MoveDirection гуманоида
        if hum and hum.MoveDirection.Magnitude > 0 then
            moveDirection = hum.MoveDirection
            -- Если вы наклоняете камеру вверх или вниз, персонаж будет лететь туда
        ...
        if moveDirection.Magnitude > 0 then
            ...
            root.FlyVelocity.Velocity = Vector3.new(0, 0.05, 0) -- Зависание на месте, если убрали палец
        end
    else
        if root:FindFirstChild("FlyVelocity") then root.FlyVelocity:Destroy() end
    end
...
-- ====================================================================
--  BLOCK 5: VISUALS (ESP), MM2 RADAR & RESET CONTROL
-- ====================================================================
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local LocalPlayer = Players.LocalPlayer

local espCheat = _G.createCheckbox(_G.pVisual, "ESPToggle", "Включить ESP (Подсветка игроков)")
local espBoxes = {}

-- Создание подсветки для каждого игрока
local function createESP(player)
    ...
    box.Parent = _G.ScreenGui

    local function update()
        if _G.Config.ESPToggle and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
    ...
    espBoxes[player] = {Box = box, Conn = conn}
end

-- Отслеживание входа и выхода игроков
Players.PlayerAdded:Connect(createESP)
for _, p in pairs(Players:GetPlayers()) do createESP(p) end
Players.PlayerRemoving:Connect(function(p)
    if espBoxes[p] then
        espBoxes[p].Box:Destroy()
        espBoxes[p].Conn:Disconnect()
        espBoxes[p] = nil
    end
end)

-- Включение радара Murder Mystery 2
...
-- Обновление списка ролей (Убийца / Шериф)
local function updateMM2Radar()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p.Character then
            ...
            -- Проверка наличия оружия в руках или в рюкзаке
            ...
            -- Выводим на панель только важные роли
            if role ~= "Innocent" then
                local lbl = Instance.new("TextLabel", _G.MM2Content)
                lbl.Size = UDim2.new(1, 0, 0, 18)
                lbl.BackgroundTransparency = 1
                lbl.Text = string.sub(p.Name, 1, 12) .. ": " .. role
...
-- Фоновый поток для периодической проверки ролей в MM2
task.spawn(function()
...
-- Кнопка полного удаления конфига и закрытия чита
