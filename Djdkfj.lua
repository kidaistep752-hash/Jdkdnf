...
-- ====================================================================
--  BLOCK 5: MAIN LOGIC (ESP, MM2 SUB-TAB, SILENT AIM & REJOIN)
-- ====================================================================
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
...
-- 1. ИНИЦИАЛИЗАЦИЯ КНОПОК В ОСНОВНОМ МЕНЮ
...
mm2SubTabBtn.TextColor3 = Color3.fromRGB(255, 65, 65)
mm2SubTabBtn.Font = Enum.Font.SourceSansBold
...
SubStroke.Color = Color3.fromRGB(255, 65, 65)
...
-- Кнопки внутри бокового меню MM2 DASHBOARD
local rolesCheat = _G.createCheckbox(_G.MM2Content, "MM2Roles", "Roles ESP (🔴/🔵/⚪)")
local gunEspCheat = _G.createCheckbox(_G.MM2Content, "MM2Gun", "Gun ESP + 3D Text")
local shotCheat = _G.createCheckbox(_G.MM2Content, "MM2Shot", "Shot Murder (Auto-Aim)")

shotCheat.OnToggle(function(state)
    _G.ShotButton.Visible = state
end)

-- 2. ЛОГИКА ФУНКЦИИ SHOT MURDER (AUTOMATIC AIM & FIRE)
...
_G.ShotButton.MouseButton1Click:Connect(function()
    ...
    if gun then
        ...
        if targetRoot and workspace.CurrentCamera then
            -- Проверка зоны видимости (Raycast)
            local cam = workspace.CurrentCamera
            local rayParams = RaycastParams.new()
            rayParams.FilterDescendantsInstances = {myChar}
            rayParams.FilterType = Enum.RaycastFilterType.Exclude
            
            local res = workspace:Raycast(cam.CFrame.Position, (targetRoot.Position - cam.CFrame.Position).Unit * 500, rayParams)
            
            if res and res.Instance:IsDescendantOf(targetRoot.Parent) then
                -- Мгновенная наводка камеры на убийцу
                cam.CFrame = CFrame.new(cam.CFrame.Position, targetRoot.Position)
                -- Имитация выстрела пистолета
                gun:Activate()
            end
        end
    end
...
-- 3. ЛОГИКА БАЗОВОГО ESP И ЦВЕТНОГО CHAMS ДЛЯ MM2
local function getMM2Color(player)
    ...
    return Color3.fromRGB(255, 255, 255) -- Невинный: Белый
end
...
local function applyPlayerVisuals(player)
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if _G.Config.MM2Roles or _G.Config.ESPToggle then
            ...
            -- Выбор цвета (Если включены Роли MM2 — красим по ролям, если базовое ESP — в фиолетовый)
...
-- 4. ЛОГИКА ПОДСВЕТКИ ПИСТОЛЕТА И 3D НАДПИСИ GUN
local function setupGunVisuals(item)
    if item:IsA("Tool") and item.Name == "Gun" then
        local handle = item:WaitForChild("Handle", 3)
        if handle and handle:IsA("BasePart") then
            
            RunService.RenderStepped:Connect(function()
                local hasHighlight = handle:FindFirstChild("GunESP")
                local hasLabel = handle:FindFirstChild("3D_GunLabel")
                
                if _G.Config.MM2Gun then
                    -- 3D Chams заливка пистолета зеленым цветом
                    if not hasHighlight then
                        ...
                        hl.Size = handle.Size * 1.2
                        hl.Adornee = handle
                        hl.Transparency = 0.4
                        hl.Parent = handle
                    end
                    
                    -- Создание 3D текста GUN над пистолетом сквозь стены
                    ...
                    if hasLabel then hasLabel:Destroy() end
                end
            end)
        end
    end
...
-- Отслеживание спавна пистолета на земле или у игроков в руках/рюкзаках
workspace.DescendantAdded:Connect(setupGunVisuals)
for _, desc in pairs(workspace:GetDescendants()) do setupGunVisuals(desc) end

-- 5. КНОПКИ НАСТРОЕК (REJOIN И СБРОС)
local RejoinBtn = Instance.new("TextButton", _G.pSettings)
RejoinBtn.Size = UDim2.new(1, 0, 0, 30)
RejoinBtn.BackgroundColor3 = Color3.fromRGB(30, 120, 30)
...
ResetBtn.MouseButton1Click:Connect(function()
    if pcall(function() return delfile end) and isfile("SimpleNeon_Config.json") then
