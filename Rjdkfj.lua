-- ====================================================================
--  BLOCK 1: CONFIG, AUTO-SAVE & CLEANUP
-- ====================================================================
...
-- Очистка старых сил и эффектов подсветки
...
-- Полный конфиг с учетом ваших правок
_G.Config = {
    ...
    XNeoFlyActive = false,
    OnePunchFling = false, -- Наша функция отталкивания
    ESPToggle = false,
    ESPColor = "WHITE",
    MM2Mod = false -- Подсветка ролей MM2
}
...
-- ====================================================================
--  BLOCK 2: UI BASE & COMPACT SYNCHRONIZED MM2 WINDOW
-- ====================================================================
...
-- Окно MM2 (теперь компактное, чисто для индикации статуса)
_G.MM2Window = Instance.new("Frame")
_G.MM2Window.Size = UDim2.new(0, 180, 0, 45)
...
-- ====================================================================
--  BLOCK 5: ONE-PUNCH FLING, MM2 ROLES CHAMS & REJOIN SYSTEM
-- ====================================================================
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
...
-- КНОПКИ ДЛЯ НОВЫХ ФУНКЦИЙ В МЕНЮ
local flingCheat = _G.createCheckbox(_G.pOther, "OnePunchFling", "OnePunchFling (Отталкивание при касании)")
local mm2Cheat = _G.createCheckbox(_G.pGame, "MM2Mod", "Подсвечивать роли игроков (Цветной Chams)")

mm2Cheat.OnToggle(function(state)
    if _G.MainFrame.Visible then _G.MM2Window.Visible = state end
end)

-- 1. ЛОГИКА ФУНКЦИИ ОТТАЛКИВАНИЯ (ONE-PUNCH FLING)
local function getClosestPlayer()
    local closest, maxDist = nil, 6 -- Радиус касания/удара для отталкивания
...
RunService.Heartbeat:Connect(function()
    ...
    if targetRoot then
        ...
        -- Сдвигаем на долю секунды в позицию цели, чтобы передать импульс удара
...
-- 2. ЛОГИКА ЦВЕТНОГО CHAMS ДЛЯ MM2
...
local function applyMM2Chams(player)
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if _G.Config.MM2Mod then
            ...
            local roleColor = getMM2RoleColor(player)
...
Players.PlayerAdded:Connect(applyMM2Chams)
for _, p in pairs(Players:GetPlayers()) do applyMM2Chams(p) end

-- 3. КНОПКИ ДЛЯ ВКЛАДКИ SETTINGS (REJOIN И СБРОС)
...
RejoinBtn.Text = "Перезайти на сервер (Rejoin)"
